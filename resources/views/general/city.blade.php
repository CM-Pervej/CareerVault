...
<section id="cityFormSection" class="flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-8 scroll-mt-6">
    <div>
        <h1 class="cv-title text-3xl">Cities</h1>
        <p class="text-base-content/60 text-sm whitespace-nowrap"> {{ $cities->count() }} cities on record </p>
    </div>

    {{-- Form Card --}}
    <div class="w-full">
        <div class="w-full">
            <form action="{{ isset($city)? route('cities.update', ['country' => $city->country->slug, 'city' => $city->slug,]): route('cities.store') }}" method="POST" autocomplete="off" class="flex flex-col sm:flex-row sm:items-start gap-3">
                @csrf
                ...
                {{-- Country --}}
                <fieldset class="w-full sm:w-56 shrink-0">
                ...
                <fieldset class="flex-1">
                    <input id="cityNameInput" type="text" name="name" class="w-full rounded-xl border border-slate-300 px-4 py-2.5 text-sm text-slate-800 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition @error('name') border-rose-400 focus:ring-rose-400 @enderror" value="{{ old('name', $city->name ?? '') }}" placeholder="e.g. Dhaka, Khulna, Chittagong">
                    @error('name')
                        <p class="text-xs text-rose-600 mt-1.5 flex items-center gap-1">
                            <i class="fa-solid fa-circle-exclamation"></i>
                            {{ $message }}
                ...
                <div class="flex gap-2 shrink-0">
                    <button type="submit" class="inline-flex items-center gap-2 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium px-4 py-2.5 shadow-sm transition">
                        <i class="fa-solid {{ isset($city) ? 'fa-check' : 'fa-plus' }}"></i>
                        {{ isset($city) ? 'Update' : 'Add City' }}
                    </button>

                    @isset($city)
                        <a href="{{ route('cities.index') }}" class="inline-flex items-center rounded-xl border border-slate-200 text-slate-600 hover:bg-slate-50 text-sm font-medium px-4 py-2.5 transition">Cancel</a>
                    @endisset
                </div>
            </form>
        </div>
...
<section class="flex flex-col lg:flex-row gap-6 items-start">
    ...
    {{-- ============================================================ --}}
    <div id="countryPanel" class="w-full lg:w-[22rem] shrink-0 lg:sticky lg:top-6 lg:self-start lg:block">
        <div class="bg-white border border-slate-200/80 rounded-2xl shadow-sm hover:shadow-md transition-shadow duration-300 overflow-hidden">
            ...
            <div class="relative px-6 pt-6 pb-5 bg-gradient-to-br from-indigo-600 via-indigo-600 to-violet-600">
                ...
                <div class="relative flex items-center justify-between mb-4">
                    ...
                    <span class="inline-flex items-center justify-center rounded-full bg-white/15 backdrop-blur px-2.5 py-1 text-xs font-semibold text-white ring-1 ring-white/20">
                        {{ $countriesWithCities->count() }}
                    </span>
                </div>
    ...
    {{-- ============================================================ --}}
    <div id="cityPanel" class="hidden lg:block bg-white border border-slate-200/80 rounded-2xl shadow-sm hover:shadow-md transition-shadow duration-300 w-full min-w-0">
        <div class="p-6">
